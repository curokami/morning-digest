...
### Collector
Source-specific Collectors normalize entries into Articles. Before MediumCollector runs, the scheduler retains preferred feeds and selects one balanced daily group of ordinary feeds; MediumCollector then reads RSS and preserves its richest available body and metadata. PythonWeeklyCollector discovers the latest Issue from the Archive and extracts individual links from configured editorial sections. ElixirLibHuntCollector discovers the latest Issue through the official feed, removes sponsorships, and extracts news and project links. CollectorGroup combines and de-duplicates their output.
